...
class InheritImportBase(models.Model):
    ...
    def import_product_pricelist(self):
        response = requests.get('%s/api/public/product.pricelist?query={*,country_group_ids{*,country_ids{name}},currency_id{*},item_ids{*,categ_id{id,name, parent_id{name}},currency_id{*}}}' %(self.base_url))
        ...
        if data.get('item_ids'):
            ...
            for item in data.get('item_ids'):
                ...
                if item.get('product_id') and item.get('product_id')!=0:
                    domain=[('remote_product_product_id','=',item.get('product_id'))]
                    find_product=self.env['product.product'].search(domain)
                    if find_product:
                        item_vals['product_id']=find_product.id   

                if item.get('product_tmpl_id') and item.get('product_tmpl_id')!=0:                
