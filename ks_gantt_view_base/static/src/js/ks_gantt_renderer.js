odoo.define("ks_gantt_view.Renderer", function (require) {
  ...
  var ksGanttRenderer = AbstractRenderer.extend({
    ...
    ks_render_data_group_by: function () {
      ...
      var ks_id_list = [];
      var ks_gantt_fields = this.ks_gantt_arch;
      var ks_group_name = this.state.groupedBy[0].split(':')[0]
      var ks_groupb_date = this.state.groupedBy.length-1

      var ks_new_group_name = this.state.groupedBy[ks_groupb_date].split(':')[0]
        ...
        for (var group_index in this.state.ks_group_by_records) {
          ...
          for (var group_lvl in this.state.groupedBy) {
            ...
            if (this.ks_fieldDetail[ks_new_group_name].type == 'datetime') {
                var ks_date_con = this.state.groupedBy[group_lvl].split(':')[0]
                var ks_project_id = this.state.groupedBy[group_lvl]
                ...
                if (this.state.groupedBy.length > 1){
                    var ks_groupb_date = this.state.groupedBy.length-2

                    var ks_new_project_id = this.state.groupedBy[ks_groupb_date].split(':')[0]
...
//                var ks_new_project_id = this.state.groupedBy[ks_groupb_date].split(':')[0]
            ...
            if (this.ks_fieldDetail[ks_new_group_name].type != 'datetime') {
                var ks_project_id = this.state.groupedBy[0].split(':')[0]
                var sub_group_name = this.state.groupedBy[group_lvl].split(':')[0];
                var ks_each_group_sub_group = this.state.groupedBy[group_lvl];
          ...
          for (var ks_task_index in this.state.records) {
            ...
            if (this.ks_fieldDetail[ks_new_group_name].type != 'datetime') {
                ...
                if ( ks_date_tf_value == true) {
                    ...
                    var new_sub_group_name = this.state.groupedBy[group_lvl]
    ...
    ks_compare_group_range_data: function (ks_count) {
      if (ks_count == 1) {
        return true;
      }
      else {
...
//       Check if parent task is available then set add parent otherwise set to false.
      ...
      if (
        ...
        var record =this.state.records.find(record => record.id === ks_gantt_data.id)
        if(!record.ks_tip){
        record.ks_tip=true
        gantt.message({
          type: "warning",
      ...
      if (
        ...
        if (each_task["ks_user_ids"]) {
          ...
          gantt.config.ks_owner_task_list.push({
            ...
            label: _t(each_task[ks_gantt_fields.ks_owner_task][1]),
...
//                        'ks_allow_parent_task': each_project_task.ks_allow_subtask,
      ...
      if (!gantt.config.ks_owner_task_list) {
        gantt.config.ks_owner_task_list = [
          { key: "0", label: _("N/A"), id: "0", text: "N/A" },
        ];
        gantt.config.ks_owner_task_dict = [];
      }
      if (
        ...
        gantt.config.ks_owner_task_list.push({
          ...
          label: _t(each_project_task.ks_owner_task[1]),
          ...
          myChart = new Chart(ctx, {
            ...
            data: {
              datasets: [
                ...
                {
                  label: _t("Real progress (predicted)"),
                  backgroundColor: "#ff5454",
                  borderColor: "#ff5454",
