...
class WebsiteOTPSettings(models.TransientModel):
    ...
    def get_values(self):
        ...
        res.update({
            ...
            #'otp_time_limit':IrDefault.get('website.otp.settings','otp_time_limit', self.otp_time_limit),
        })
        return res

# ## make Emipro woocommerce compatible with webkul
#     woo_import_order_status_ids = fields.Many2many(store=False)
#     woo_user_ids = fields.Many2many(store=False)
#     woo_financial_status_ids = fields.Many2many(store=False)
#     woo_analytic_tag_ids = fields.Many2many(store=False)
# ## make Emipro woocommerce compatible with webkul 