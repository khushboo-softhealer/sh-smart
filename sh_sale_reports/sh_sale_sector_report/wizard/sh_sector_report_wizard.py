...
class SectionReportWizard(models.TransientModel):
    ...
    invoice_status = fields.Selection([
        ...
        ], string='Invoice Status(so)',default='to invoice',required = True)
    ...
    def print_section_report(self):
        ...
        self.env.cr.execute("""select name,from_time,to_time from sh_sector order by sequence;""")
        ...
        worksheet.write_merge(0, 1, 9, 12, 'Invoice Status : '+ dict(self._fields['invoice_status'].selection).get(self.invoice_status), header_record)

        listToStr = ''
        if self.sale_team_ids:
            listToStr = ', '.join([str(elem) for elem in self.sale_team_ids.mapped('name')])

            worksheet.write_merge(0, 1, 13, 16, 'Sales Channel : '+ listToStr, header_record)
...
#             col = 19
            ...
            worksheet.write_merge(row, row, col, col+1, str(key)+' Total Sales', right_bold)
