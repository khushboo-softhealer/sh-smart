...
class HelpdeskTicketFeedbackController(http.Controller):
    ...
    def team_data(self):
        team_obj = request.env['sh.helpdesk.team'].search([])
        res_list = {}
        for rec in team_obj:
            res = {}
    ...
    def get_user_group(self, **post):
        ...
        if support_user == True and team_leader == False and manager == False:
            dic.update({
                'user': '1'
            })
        elif support_user == True and team_leader == True and manager == False:
            dic.update({
                'leader': '1'
            })
        elif support_user == True and team_leader == True and manager == True:
            dic.update({
                'manager': '1'
            })
        return json.dumps(dic)
    ...
    def get_ticket_counter_data(self, **kw):
        ticket_obj = request.env['sh.helpdesk.ticket'].search(
        ...
        for stage in company_id.dashboard_filter:
            ...
            if kw.get('filter_date') == 'today':
                ...
                if kw.get('date_start') and kw.get('date_end'):
                    ...
                    doman.append(tuple(dt_flt2))
            if kw.get('team') and int(kw.get('team')) != 0:
                doman.append(('team_id', '=', int(kw.get('team'))))
            elif kw.get('team') and int(kw.get('team')) == 0:
                if request.env.user.has_group('sh_helpdesk.helpdesk_group_team_leader') == True and request.env.user.has_group('sh_helpdesk.helpdesk_group_user') == True and request.env.user.has_group('sh_helpdesk.helpdesk_group_manager') == False:
                    team_ids = request.env['sh.helpdesk.team'].sudo().search(
                        ['|', ('team_head', '=', request.env.user.id), ('team_members', 'in', [request.env.user.id])])
                    doman.append(('|'))
                    doman.append(('team_id', 'in', team_ids.ids))
                    doman.append(('team_id', '=', False))
                elif request.env.user.has_group('sh_helpdesk.helpdesk_group_team_leader') == False and request.env.user.has_group('sh_helpdesk.helpdesk_group_user') == True and request.env.user.has_group('sh_helpdesk.helpdesk_group_manager') == False:
                    team_ids = request.env['sh.helpdesk.team'].sudo().search(
            ...
            if kw.get('team_leader') and int(kw.get('team_leader')) != 0:
                doman.append(('team_head', '=', int(kw.get('team_leader'))))
            elif kw.get('team_leader') and int(kw.get('team_leader')) == 0:
                if request.env.user.has_group('sh_helpdesk.helpdesk_group_team_leader') == True and request.env.user.has_group('sh_helpdesk.helpdesk_group_user') == True and request.env.user.has_group('sh_helpdesk.helpdesk_group_manager') == False:
                    doman.append(('|'))
                    doman.append(('|'))
                    doman.append(('|'))
                    ...
                    doman.append(('user_id', '=', False))
                    doman.append(('sh_user_ids', '=', False))
            if kw.get('user_id') and int(kw.get('user_id')) != 0:
                doman.append(('|'))
                doman.append(('user_id', '=', int(kw.get('user_id'))))
                doman.append(('sh_user_ids', 'in', [int(kw.get('user_id'))]))
            elif kw.get('user_id') and int(kw.get('user_id')) == 0:
                if request.env.user.has_group('sh_helpdesk.helpdesk_group_team_leader') == True and request.env.user.has_group('sh_helpdesk.helpdesk_group_user') == True and request.env.user.has_group('sh_helpdesk.helpdesk_group_manager') == False:
                    doman.append(('|'))
                    doman.append(('|'))
                    doman.append(('|'))
            ...
            if search_tickets:
                for ticket in search_tickets:
                    ...
                    write_date = datetime.strftime(
                        ticket.write_date, "%Y-%m-%d %H:%M:%S")
                    ticket_dic = {
                        'ticket_id': ticket.id,
            ...
            search_stage = request.env['sh.helpdesk.stages'].search([
                ('id', '=', stage.id)
            ], limit=1)
            if search_stage:
    ...
    @http.route('/open-ticket', type='http', auth="public")
    def open_tickets(self, **kw):
        dashboard_id = request.env['sh.ticket.dashboard'].sudo().search(
            [('id', '=', 1)], limit=1)
