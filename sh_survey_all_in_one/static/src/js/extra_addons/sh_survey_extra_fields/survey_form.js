    ...
    publicWidget.registry.SurveyFormWidget.include({
        ...
        */
            ...
            $form.find('[data-question-type]').each(function () {
                ...
                switch ($input.data('questionType')) {
                    ...
                    case 'que_sh_address':
                        ...
                        break;
                    case 'que_sh_qrcode':
                        if (questionRequired && !$input.val() && !$input.val()) {
                            errors[questionId] = constrErrorMsg;
                        }
                        break;
                    case 'que_sh_barcode':
        ...
        _focusOnFirstInput: function () {
			...
			if (this.$("input[type='range'].o_survey_question_email_box").length ){   
				this.$("input[type='range'].o_survey_question_email_box").trigger('input')
     	    }
        },
