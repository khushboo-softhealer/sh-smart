...
class SalesProductProfitWizard(models.TransientModel):
    ...
    def print_xls_report(self):
        ...
        if self.report_by == 'customer':
            if order_dic_by_customers:
                for customer in order_dic_by_customers.keys():
                    worksheet.write_merge(
                        row, row, 0, 8, customer, bold_center)
                    row = row+2
                    total_cost = 0.0
                    ...
                    worksheet.write(row, 6, "Discount", bold)
                    ...
                    for rec in order_dic_by_customers[customer]:
                        ...
                        sale_price = (rec.get('sale_price',0.0)*rec.get('qty',0.0) - rec.get('sale_price',0.0)*rec.get('qty',0.0)*rec.get('discount')/100)
                        worksheet.write(
                            row, 5, "{:.2f}".format(sale_price), center)
                        worksheet.write(row, 6, "{:.2f}".format(rec.get('discount')), center)
                        profit = ((rec.get('sale_price',0.0)*rec.get('qty',0.0))-rec.get('sale_price',0.0)*rec.get('qty',0.0)*rec.get('discount')/100) - (rec.get('cost',0.0)*rec.get('qty',0.0))
                        worksheet.write(
            ...
            if order_dic_by_products:
                for product in order_dic_by_products.keys():
                    worksheet.write_merge(
                        row, row, 0, 8, product, bold_center)
                    row += 2
                    total_cost = 0.0
                    total_sale_price = 0.0
                    total_profit = 0.0
                    ...
                    for rec in order_dic_by_products[product]:
                        ...
                        profit = ((rec.get('sale_price',0.0)*rec.get('qty',0.0))-rec.get('sale_price',0.0)*rec.get('qty',0.0)*rec.get('discount')/100) - (rec.get('cost',0.0)*rec.get('qty',0.0))
                        worksheet.write(
                            row, 6, "{:.2f}".format(rec.get('discount')), center)
                        worksheet.write(
                            row, 7, "{:.2f}".format(profit), center)
                        if sale_price != 0.0:
                            margin = (profit/sale_price)*100
            ...
            worksheet.write(row, 7, "Discount", bold)
            ...
            if both_order_list:
                for order in both_order_list:
                    ...
                    sale_price = (order.get('sale_price',0.0)*order.get('qty',0.0) - order.get('sale_price',0.0)*order.get('qty',0.0)*order.get('discount')/100)
                    worksheet.write(
                        row, 6, "{:.2f}".format(sale_price), center)
                    profit = ((order.get('sale_price',0.0)*order.get('qty',0.0))-order.get('sale_price',0.0)*order.get('qty',0.0)*order.get('discount')/100) - (order.get('cost',0.0)*order.get('qty',0.0))
                    worksheet.write(
                        row, 7, "{:.2f}".format(order.get('discount')), center)
                    worksheet.write(
        ...
        attachment = IrAttachment.search([('name', '=',
                'Sales_Product_Profit'), ('type', '=', 'binary'),
