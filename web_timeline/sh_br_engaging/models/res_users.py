...
class res_users(models.Model):
    ...
    def systray_get_firebase_notifications(self):
        notifications = self.env['sh.br.engage.push.notification'].sudo().search([('user_id','=',self.env.uid)],limit=25, order='msg_read,id desc')
        unread_notifications = self.env['sh.br.engage.push.notification'].sudo().search([('user_id','=',self.env.uid),('msg_read','=',False)])
        data_notifications = []
