...
class KsDashboardNinjaBoard(models.Model):
    ...
    def update_all_field_ids(self, item):
        field_ordering_data = json.loads(item['ks_many2many_field_ordering'])

        model_name = item.get('ks_model_id')
        if not model_name:
            return "Model name not found in item data."
        ...
        all_field_names = []
        for key, value in field_ordering_data.items():
            if key.endswith('_name') and isinstance(value, list):
                all_field_names.extend(value)

        fields = self.env['ir.model.fields'].search([
            ('model_id', '=', model.id),
            ('name', 'in', all_field_names)
        ])
        field_id_mapping = {field.name: field.id for field in fields}

        for key, value in field_ordering_data.items():
            if key.endswith('_name') and isinstance(value, list):
                base_key = key[:-5]
                if base_key in field_ordering_data:
                    field_ordering_data[base_key] = [
                        field_id_mapping.get(field_name, None) for field_name in value
                    ]

        item['ks_many2many_field_ordering'] = json.dumps(field_ordering_data, indent=4)
        return item

    def ks_prepare_item(self, item):
        try:
            self.update_all_field_ids(item)
