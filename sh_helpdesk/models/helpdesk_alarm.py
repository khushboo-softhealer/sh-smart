...
class TicketAlarm(models.Model):
    ...
    def _run_ticket_reminder(self):        
        ...
        if self.env.user.company_id.sh_display_ticket_reminder:
                                ...
                                if alarm.type == 'popup':
                                    ...
                                    if fields.Date.today() == deadline_date.date() and deadline_date.hour == now.hour and deadline_date.minute == now.minute:
                                        ...
                                        if ticket.sub_category_id:
                                            message += "<strong>Sub Category :" + \
                                                str(ticket.sub_category_id.name) + \
                                                "</strong></br>"
                                        if ticket.priority:
                                            message += "<strong>Priority :" + \
                                        ...
                                        if ticket.team_id:
                                            message += "<strong>Team :" + \
                                                ...
                                                "</strong></br>"
                                        if ticket.team_head:
                                            message += "<strong>Team Head :" + \
                                                str(ticket.team_head.name) + \
                                                "</strong></br>"
                                        if ticket.user_id:
                                            message += "<strong>Assigned To :" + \
                                        ...
                                        if len(tags) > 0:
                                            ...
                                            message += '</br>'
                                        message += '</br></br>'
                                        message += "<h2><u>Contact & Products Information</u></h2></br></br>"
                                        if ticket.sh_edition_id:
                                            message += "<strong>Odoo Edition :" + \
                                                str(ticket.sh_edition_id.name) + \
                                                "</strong></br>"
                                        products = []
                                        ...
                                        if ticket.partner_id:
                                            message += "<strong>Partner :" + \
                                                str(ticket.partner_id.name) + \
                                                "</strong></br>"
                                        if ticket.person_name:
                                            message += "<strong>Person Name :" + \
                                    ...
                                    partners = []
                                    if ticket.team_head:
                                        if str(ticket.team_head.partner_id.id) not in partners:
                                            partners.append(
                                                str(ticket.team_head.partner_id.id))
                                    if ticket.user_id:
                                        if str(ticket.user_id.partner_id.id) not in partners:
                                            partners.append(
                                                str(ticket.user_id.partner_id.id))
                                    if ticket.sh_user_ids:
                                        for user_id in ticket.sh_user_ids:
                                            if str(user_id.partner_id.id) not in partners:
                                                partners.append(
                                                    str(user_id.partner_id.id))
                                    if fields.Date.today() == deadline_date.date() and deadline_date.hour == now.hour and deadline_date.minute == now.minute:
                                        reminder_template = self.env.ref(
                                            'sh_helpdesk.sh_ticket_reminder_mail_template')
                                        if reminder_template and len(partners) > 0:
                                            partner_ids = ','.join(partners)
                                            reminder_template.sudo().write({
                                                'partner_to': partner_ids
                                            })
                                            reminder_template.sudo().send_mail(ticket.id, force_send=True)
