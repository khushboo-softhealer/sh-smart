...
class KsProjectTask(models.Model):
    ...
    def ks_update(self,startdate,enddate):
        query="""
            UPDATE project_task set ks_start_datetime = %(startdate)s ,ks_end_datetime = %(enddate)s Where id= %(id)s
        """
        self.env.cr.execute(query,{'startdate':startdate,'enddate':enddate,'id':self.id})
        ...
        if self.ks_schedule_mode == 'auto':
            ...
            if not task_link:
                ...
                if self.ks_constraint_task_type == 'asap':
                    if self.project_id.ks_project_start < self.ks_start_datetime:
                        self.ks_update(self.project_id.ks_project_start,self.project_id.ks_project_start + ks_duration)
                        # self.ks_start_datetime = self.project_id.ks_project_start
                        # self.ks_end_datetime = self.project_id.ks_project_start + ks_duration
                    else:
                        self.ks_update(self.project_id.ks_project_start + ks_duration, self.project_id.ks_project_start)
                        # self.ks_end_datetime = self.project_id.ks_project_start + ks_duration
                        # self.ks_start_datetime = self.project_id.ks_project_start
                ...
                if self.ks_constraint_task_type == 'alap':
                    ...
                    if ks_closest_task:
                        self.ks_update(self.ks_end_datetime - ks_duration, ks_closest_task)
            ...
            if len(task_link) == 1 and task_link.ks_task_link_type == "0":
                ...
                if task_link.ks_source_task_id.ks_end_datetime < self.ks_start_datetime:
                    self.ks_update(task_link.ks_source_task_id.ks_end_datetime - ks_duration, task_link.ks_source_task_id.ks_end_datetime + ks_duration)
                    ...
                    self.ks_update(task_link.ks_source_task_id.ks_end_datetime,
                                   task_link.ks_source_task_id.ks_end_datetime + ks_duration)
            ...
            if len(task_link) == 1 and task_link.ks_task_link_type == "1":
                ...
                if task_link.ks_source_task_id.ks_start_datetime < self.ks_start_datetime:
                    self.ks_update(task_link.ks_source_task_id.ks_start_datetime,
                                   task_link.ks_source_task_id.ks_start_datetime + ks_duration)
            ...
            if len(task_link) == 1 and task_link.ks_task_link_type == "2":
                ks_duration = self.ks_end_datetime - self.ks_start_datetime
                if task_link.ks_source_task_id.ks_end_datetime < self.ks_start_datetime:
                    self.ks_update(task_link.ks_source_task_id.ks_end_datetime - ks_duration,
                                   task_link.ks_source_task_id.ks_end_datetime)
            ...
            if len(task_link) == 1 and task_link.ks_task_link_type == "3":
                ...
                if task_link.ks_source_task_id.ks_start_datetime < self.ks_end_datetime:
                    self.ks_update(task_link.ks_source_task_id.ks_start_datetime - ks_duration,
                                   task_link.ks_source_task_id.ks_start_datetime)
