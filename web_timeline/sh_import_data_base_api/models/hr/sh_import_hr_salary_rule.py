...
class InheritImportHrPayrollStructure(models.Model):
    ...
    def prepare_hr_salary_rule_vals(self,data):
        ...
        hr_salary_rule_vals={
            ...
            'amount_select':data['amount_select'],
        }
        ...
        if data.get('category_id') and data['category_id']['id'] and data['category_id']['id']!=0:
            domain = [('remote_hr_salary_rule_category_id', '=', data['category_id']['id'])]
            find_rule_cat = self.env['hr.salary.rule.category'].search(domain)
            if find_rule_cat:
                hr_salary_rule_vals['category_id'] = find_rule_cat.id
            else:
                rules_cat_vals=self.process_hr_salary_rule_category(data['category_id'])
                rule_cat_id=self.env['hr.salary.rule.category'].create(rules_cat_vals)
                if rule_cat_id:
