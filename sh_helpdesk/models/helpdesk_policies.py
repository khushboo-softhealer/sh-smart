...
class HelpdeskSLAPolicies(models.Model):
    ...
    sh_team_id = fields.Many2one(
        'sh.helpdesk.team', 'Helpdesk Team', required=True)
    sh_ticket_type_id = fields.Many2one(
        'sh.helpdesk.ticket.type', 'Helpdesk Team Type')
    sh_sla_target_type = fields.Selection([('reaching_stage', 'Reaching Stage'), (
        'assign_to', 'Assigned To')], default='reaching_stage', string='SLA Target Type')
    sh_stage_id = fields.Many2one('sh.helpdesk.stages', string='Reach Stage')
    ...
    def _compute_helpdesk_ticket_sla(self):
        for record in self:
            ...
            tickets = self.env['sh.helpdesk.ticket'].search(
                [('id', '=', record.sh_team_id.id)], limit=None)
